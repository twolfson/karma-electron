...
function ElectronBrowser(baseBrowserDecorator, args, karmaBasePath, karmaUrlRoot) {
  ...
  this._getOptions = function (url) {
    var retArr = [__dirname + '/electron-launcher.js']
      ...
      .concat(args.browserWindowOptions ?
        [ '--browser-window-options', serializeJSON(args.browserWindowOptions) ] :
        []
      )
      .concat(args.entry ? [ '--entry', args.entry ] : [])
      .concat(args.devTools ? [ '--dev-tools' ] : [])
    ;
    return retArr;
  };
}
...
function serializeJSON(value) {
  try {
    ...
    console.error(
      'Unable to serialize BrowserWindow options, this likely indicates ' +
      'you have passed non-JSON serializable values like functions or objects ' +
      'with circual references.'
    );
