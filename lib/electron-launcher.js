...
program.option('--browser-window-options [json]', 'Custom BrowserWindow configuration.', null)
program.option('--dev-tools', 'Open DevTools automatically with each window.', false)
...
if (program.devTools) {
  app.on('browser-window-created', function(e, browserWindow) {
...
app.on('ready', function handleReady () {
  var customOptions;
  if (program.browserWindowOptions) {
    try {
      customOptions = JSON.parse(program.browserWindowOptions);
    }
    catch (e) {
      console.error(
        'Unable to deserialize BrowserWindow options, this likely indicates ' +
        'you have passed non-JSON serializable values like functions.'
      );
  ...
  var browserWindow = new BrowserWindow(Object.assign({}, {
    show: !!program.show
  }, customOptions));
