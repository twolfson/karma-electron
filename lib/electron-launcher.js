...
program.option('--abc', 'Load extended options from `karma-electron-options.json` in `user-data-dir`');
...
// Exploring https://github.com/electron/windows-installer/issues/167
...
async.parallel([
  function loadExtendedOptions (cb) {
    ...
    if (true) {
      fs.readFile(path.join(program.userDataDir, 'karma-electron-options.json'), 'utf8', function handleReadFile (err, extendedOptionsStr) {
...
console.log('mid5');
  browserWindow.loadURL(decodeURIComponent(program.url), loadURLOptions);
});
